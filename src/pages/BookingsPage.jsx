...
export default function BookingsPage() {
  ...
  const { data: prenotazioni = [], isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Identifica prenotazioni 'confermata' o 'in_corso' con fine_sosta già passata
      const now = new Date();
      const daCompletare = rows.filter(
        p => (p.stato === 'confermata' || p.stato === 'in_corso') &&
             new Date(p.fine_sosta) <= now
      );

      // Aggiorna lo stato a 'completata' in background (fire-and-forget)
      if (daCompletare.length > 0) {
        const ids = daCompletare.map(p => p.id_prenotazione);
        supabase
          .from('prenotazioni')
          .update({ stato: 'completata' })
          .in('id_prenotazione', ids)
          .then(({ error: updateError }) => {
            if (updateError) {
              console.error('[BookingsPage] Errore completamento automatico:', updateError);
            } else {
              // Ricarica per riflettere lo stato aggiornato
              queryClient.invalidateQueries({ queryKey: ['prenotazioni', user?.id] });
            }
          });

        // Aggiorna lo stato localmente in modo ottimistico
        return rows.map(p =>
          ids.includes(p.id_prenotazione) ? { ...p, stato: 'completata' } : p
        );
      }
    ...
    <div className="p-4 lg:p-8 max-w-4xl mx-auto space-y-6">
      ...
      <Tabs value={filter} onValueChange={setFilter}>
        <TabsList className="flex flex-wrap">
          ...
          <TabsTrigger value="confermata">Attive</TabsTrigger>
