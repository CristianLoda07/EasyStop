...
export default function AdminUsers() {
  ...
  // DEBUG: log utenti e utenti attivi
  console.log('TUTTI UTENTI:', utenti);

  // Mostra tutti gli utenti attivi, anche senza email
  const activeUsers = utenti.filter(u => u.is_active);

  // Permetti la ricerca anche se l'email è vuota
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Badge variant="secondary" className="text-sm">
            <Users className="w-4 h-4 mr-1" /> {activeUsers.length} attivi
          </Badge>
            ...
            <TableBody>
              {!isLoading && filtered.length === 0 ? (
                ...
                <TableRow key={u.id}>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center">
                        <span className="text-sm font-bold text-primary">
                          {u.full_name?.[0]?.toUpperCase() || (u.email ? u.email[0]?.toUpperCase() : '-')}
                        </span>
                      </div>
                  ...
                  <TableCell>
                    <Badge variant="default" className="bg-green-600 hover:bg-green-600">Online</Badge>
                  </TableCell>
                  <TableCell>
                    <Button 
                      variant="ghost" 
