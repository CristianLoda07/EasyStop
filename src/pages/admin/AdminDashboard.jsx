...
import { STATO_LABELS, CHART_COLORS, getColorByOccupancy, OCCUPANCY_COLOR_CLASSES, isPrenotazioneAttiva } from '@/lib/greenUtils';
...
export default function AdminDashboard() {
  const { data: prenotazioni = [] } = useQuery({
    ...
    queryFn: async () => {
      const prenotazioniRaw = await base44.entities.Prenotazione.list('-created_date', 500);
      const now = new Date();
      const daAggiornare = (prenotazioniRaw || []).map((p) => ({
        ...p,
        statoEffettivo: p.stato === 'cancellata' || p.stato === 'completata'
          ? p.stato
          : (isPrenotazioneAttiva(p, now) ? 'in_corso' : (now >= new Date(p.fine_sosta) ? 'completata' : 'confermata')),
      })).filter((p) => p.statoEffettivo !== p.stato);

      if (daAggiornare.length > 0) {
        await Promise.all(
          daAggiornare.map((p) => supabase
            .from('prenotazioni')
            .update({ stato: p.statoEffettivo })
            .eq('id_prenotazione', p.id_prenotazione || p.id))
        );
      }

      return (prenotazioniRaw || []).map((p) => ({
        ...p,
        stato: p.stato === 'cancellata' || p.stato === 'completata'
          ? p.stato
          : (isPrenotazioneAttiva(p, now) ? 'in_corso' : (now >= new Date(p.fine_sosta) ? 'completata' : 'confermata')),
      }));
    },
  ...
  const today = prenotazioni.filter(p => {
    const d = new Date(p.inizio_sosta);
    return d.toDateString() === now.toDateString();
  });

  const prenotazioniAttive = prenotazioni.filter(p => {
    return isPrenotazioneAttiva(p, now);
  });

  const occupazioneParcheggi = parcheggi
    .map(p => {
      const capacita = Number(p.capacita) || 0;
      const idParcheggio = p.id_park || p.id;
      const postiOccupati = prenotazioniAttive.filter(prenotazione => (prenotazione.id_park || prenotazione.parcheggio_id) === idParcheggio).length;
      const postiLiberi = Math.max(0, capacita - postiOccupati);
      const percentualeOccupazione = capacita > 0 ? Math.round((postiOccupati / capacita) * 100) : 0;

      return {
        id: idParcheggio,
        name: p.park_name || 'Sconosciuto',
        postiLiberi,
        capacita,
  ...
  prenotazioni.forEach(p => {
    // Trova il parcheggio corrispondente
    const idParcheggio = p.id_park || p.parcheggio_id;
    const parcheggio = parcheggi.find(pk => (pk.id_park || pk.id) === idParcheggio);
    const name = parcheggio ? (parcheggio.park_name || 'Sconosciuto') : 'Sconosciuto';
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
            {occupazioneParcheggi.map(p => {
              const colorKey = getColorByOccupancy(p.postiLiberi, p.capacita);
              const colorClasses = OCCUPANCY_COLOR_CLASSES[colorKey];
              return (
                <div key={p.id} className="flex items-center gap-3">
                  <span className="text-sm font-medium min-w-40 truncate">{p.name}</span>
                  <div className="flex-1 bg-muted rounded-full h-3">
                    <div
                      className={`h-3 rounded-full transition-all ${colorClasses.bg}`}
                      style={{ width: `${p.percentualeOccupazione}%` }}
                    />
                  </div>
                  <span className="text-sm text-muted-foreground w-16 text-right">{p.percentualeOccupazione}%</span>
                </div>
              );
            })}
          </div>
