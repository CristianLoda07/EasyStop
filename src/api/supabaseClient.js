...
// Determina lo storage disponibile: alcuni PC scolastici bloccano localStorage.
// In quel caso si usa sessionStorage come fallback (la sessione dura fino alla chiusura del tab).
function detectAvailableStorage() {
  try {
    const testKey = '__easystop_storage_test__';
    ...
    try {
      ...
      console.warn(
        '[EasyStop] localStorage non disponibile (PC scolastico?): ' +
        'uso sessionStorage. La sessione durerà fino alla chiusura del tab.'
      );
      return sessionStorage;
    } catch {
      console.error('[EasyStop] Nessuno storage disponibile: sessione non persistita.');
      return undefined;
    }
  }
}
...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    ...
    storage: detectAvailableStorage(),
  },
});
...
function dbToApp(row, authEmail = '') {
  ...
  return {
    ...
    nome:        row.nome,
    cognome:     row.cognome,
    // Priorità: email da auth, poi email salvata nel DB, poi stringa vuota
...
export const auth = {
  register: async (email, password, full_name) => {
    try {
      ...
      // Se identities è vuoto, l'email è già registrata
      ...
      // Restituisce anche il flag per sapere se l'email deve essere confermata
      return {
        user: authData.user,
  ...
  login: async (email, password) => {
    try {
      ...
      // Errore nativo Supabase (verifica abilitata nel dashboard)
      if (error) {
        if (error.message === 'Email not confirmed') {
          const notConfirmedError = new Error('Email non verificata. Controlla la tua casella email e clicca il link di conferma.');
      ...
      // Controllo esplicito anche se Supabase non blocca (verifica disabilitata nel dashboard)
      if (data.user && !data.user.email_confirmed_at) {
        // Forza il logout per non lasciare la sessione attiva
      ...
      if (data.user) {
        // Aggiorna isActive a TRUE e salva email al login
        await supabase
          .from(PROFILI_TABLE)
          .update({ isActive: true, Email: data.user.email })
          .eq('id_user', data.user.id);
      }

      return data.user;
    } catch (error) {
      if (error.code === 'email_not_confirmed') throw error;
