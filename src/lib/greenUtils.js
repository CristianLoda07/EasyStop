...
/**
 * Restituisce lo stato effettivo della prenotazione in base all'orario corrente.
 * La prenotazione è considerata attiva (in_corso) dall'inizio alla fine della sosta.
 */
export function getStatoEffettivoPrenotazione(prenotazione, referenceDate = new Date()) {
  if (!prenotazione) return 'confermata';
  if (prenotazione.stato === 'cancellata' || prenotazione.stato === 'completata') {
    return prenotazione.stato;
  }

  const inizioSosta = new Date(prenotazione.inizio_sosta);
  const fineSosta = new Date(prenotazione.fine_sosta);
  if (Number.isNaN(inizioSosta.getTime()) || Number.isNaN(fineSosta.getTime())) {
    return prenotazione.stato || 'confermata';
  }

  if (referenceDate >= fineSosta) return 'completata';
  if (referenceDate >= inizioSosta) return 'in_corso';
  return 'confermata';
}

/**
 * Indica se la prenotazione è attualmente attiva.
 */
export function isPrenotazioneAttiva(prenotazione, referenceDate = new Date()) {
  return getStatoEffettivoPrenotazione(prenotazione, referenceDate) === 'in_corso';
}
