...
export default function ParkingFormDialog({ parking, open, onClose, onSave }) {
  ...
  useEffect(() => {
    if (parking) {
      setForm({
        ...
        posti_liberi: parking.posti_liberi ?? '',
  ...
  // Nuova funzione per aggiornare indirizzo/lat/lng dal componente mappa/autocomplete
  const handleAddressMapChange = (vals) => {
    setForm(f => ({ ...f, ...vals }));
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Autocomplete + mappa per indirizzo/lat/lng */}
          <div className="space-y-2">
            <Label>Indirizzo e posizione</Label>
